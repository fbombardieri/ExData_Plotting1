#Setting working directory containing the source file
#setwd("...")

filename <- "household_power_consumption.txt"

#If file does not exists then it is downloaded and unzipped in the working directory
if(!file.exists(filename)) {
  temp <- tempfile()
  file.url <- "https://d396qusza40orc.cloudfront.net/exdata%2Fdata%2Fhousehold_power_consumption.zip"
  download.file(file.url, temp)
  unzip(temp)
  unlink(temp)
}

#Reading data
col.classes <- c("character", "character", rep("numeric", 7))
sourcedata <- read.csv(filename, sep= ";", colClasses= col.classes, na.strings=c("?"))

#Selecting dates
sel.dates <- subset(sourcedata, as.Date(Date, "%d/%m/%Y") == "2007-02-01" | as.Date(Date, "%d/%m/%Y") == "2007-02-02" ) 

#Converting dates
sel.dates2 <- cbind.data.frame(
            strptime( paste(sel.dates$Date, " ", sel.dates$Time) , "%d/%m/%Y %H:%M:%S"), 
            sel.dates[, -c(1,2)]
            )
names(sel.dates2)[1] <- "Datetime"

#Setting locale
Sys.setlocale("LC_TIME", "English")

#Opening PNG output
png("plot4.png", 
    width = 480, 
    height = 480,
    bg= "transparent",
    antialias= "cleartype")


#Setting parameters
par(mfrow= c(2, 2))


#Plotting 1,1
plot(sel.dates2$Datetime, sel.dates2$Global_active_power,
     pch= ".",
     xlab= "",
     ylab= "Global Active Power",
     main= "",
     type= "n")

lines(sel.dates2$Datetime, sel.dates2$Global_active_power)


#Plotting 1,2
plot(sel.dates2$Datetime, sel.dates2$Voltage,
     pch= ".",
     xlab= "datetime",
     ylab= "Voltage",
     main= "",
     type= "n")

lines(sel.dates2$Datetime, sel.dates2$Voltage)


#Plotting 2,1
plot(sel.dates2$Datetime, sel.dates2$Sub_metering_1,
     pch= ".",
     xlab= "",
     ylab= "Energy sub metering",
     main= "",
     type= "n"
)

lines(sel.dates2$Datetime, sel.dates2$Sub_metering_1, col= "black")
lines(sel.dates2$Datetime, sel.dates2$Sub_metering_2, col= "red")
lines(sel.dates2$Datetime, sel.dates2$Sub_metering_3, col= "blue")
legend("topright", lwd= 2, bty= "n", cex= 1,
       col = c("black", "red", "blue"), 
       legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))


#Plotting 2,2
plot(sel.dates2$Datetime, sel.dates2$Global_reactive_power,
     pch= ".",
     xlab= "datetime",
     ylab= "Global_reactive_power",
     main= "",
     type= "n"
)

lines(sel.dates2$Datetime, sel.dates2$Global_reactive_power)


dev.off()
